...
if __name__ == '__main__':
    env = TroyEnv()
    rider1 = Agent(env.observation_space, env.action_space, 1.0)
    ...
    print('episode : loss_rider1_avg : loss_rider2_avg : reward_rider1_avg'
    ...
    for episode in range(episodes):
        ...
        print(f'{episode:5d} : {np.mean(losses1):5.2f}'
              f' : {np.mean(losses2):5.2f} : {np.array(rewards)[:,0].mean():5f}'
